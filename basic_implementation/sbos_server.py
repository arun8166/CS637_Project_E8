...
app = FastAPI(title="Playground-like SBOS")

# ---------------- Persistence (transaction log, constraints, validators, timeseries) ----------------
def db():
...
# ---------------- Global state, caches, and update lock ----------------
G = Graph()
PROFILES = {}
POLICY = {}
USERS = {}
...
CAP_CACHE: Dict[str, Dict[str, List[str]]] = {}  
UPDATE_LOCK = threading.RLock()  

def load_all():
    global G, PROFILES, POLICY, USERS, IRI2LABEL, LABEL2IRI, CAP_CACHE
    with UPDATE_LOCK:
        G = Graph()
        ...
        # load constraints and validator defaults
        with DB:
...
# ---------------- Helpers: Brick/labels, SPARQL, type-checking ----------------
...
def require_typed_label(label: str, brick_type_qname: str):
    # Verify label corresponds to an entity of the required Brick class
    rows = G.query(f"""
      PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
      PREFIX brick: <https://brickschema.org/schema/Brick#>
      SELECT ?x WHERE {{
        ?x a {brick_type_qname} ; rdfs:label "{label}" .
      }}
    """)
    ok = any(True for _ in rows)
    if not ok:
        raise HTTPException(400, f"Typed argument check failed: label '{label}' is not a {brick_type_qname}")

# ---------------- Permission Manager (profiles + delegation + caching) ----------------
...
APPS: Dict[str, Dict] = {}         # app_instance_id -> {manifest, pid, key, caps, user, rate state}
...
def compute_caps(man: Manifest) -> Dict[str,List[str]]:
    ...
    read_final = [i for i in app_read if i in uread]
    write_final = [i for i in app_write if i in uwrite]
    return {"read": read_final, "write": write_final}

# ---------------- Resource Proxy (simulated driver) + transaction and timeseries logging ----------------
...
# ---------------- Resource Regulator (validator queues, default-deny, constraints) ----------------
...
# ---------------- Live resource monitor (mitigation actions) ----------------
...
# ---------------- App Steward (manifests, rate limits, lifecycle) ----------------
...
def list_instances():
    ...
    for aid, inst in APPS.items():
        out.append({
            ...
            "user": inst["user"],
            "caps": {
                "read": [IRI2LABEL.get(i,i) for i in inst["caps"]["read"]],
...
# ---------------- Public/Steward/Admin Endpoints ----------------
@app.get("/health")
def health():
...
def register_app(manifest: ManifestIn):
    ...
    CAP_CACHE[info["app_instance_id"]] = compute_caps(man)
    return info
...
def capabilities(x_app_key: Optional[str] = Header(default=None)):
    ...
    caps = inst["caps"]
    pts = sorted(set(caps["read"] + caps["write"]))
    return {"app_instance": aid, "points": [{"iri": i, "label": IRI2LABEL.get(i,i)} for i in pts]}

@app.get("/read")
def read(point_label: str, x_app_key: Optional[str] = Header(default=None, convert_underscores=False)):
    aid, inst = get_app_instance_from_key(x_app_key)
...
@app.post("/write")
def write(req: WriteReq, x_app_key: Optional[str] = Header(default=None, convert_underscores=False)):
    with UPDATE_LOCK:
        aid, inst = get_app_instance_from_key(x_app_key)
